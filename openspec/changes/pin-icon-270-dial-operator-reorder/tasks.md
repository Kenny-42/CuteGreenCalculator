...
- [ ] 1.1 Confirm `star.png`/`star_pressed.png` are present in
...
- [ ] 1.2 Point `AlwaysOnTopToggleStyle`'s `Tag`/`PixelButton.CheckedSource`
...
- [ ] 2.1 Confirm `mid_green_button.png`/`mid_green_button_pressed.png`
      are present in `Assets/` (already copied).
- [ ] 2.2 In `Styles/PixelButtonStyles.xaml`, replace
...
- [ ] 2.3 Delete the now-unreferenced
...
- [ ] 3.3 Verify the final row order is: row3 `C CE +/- ÷`, row4
      `7 8 9 ×`, row5 `4 5 6 −`, row6 `1 2 3 +`, row7 `0 . √ =`.

## 4. Spec sync + verification

- [ ] 4.1 `dotnet build` and `dotnet test` clean.
- [ ] 4.2 Run the app (see `/run`) and visually confirm: the always-on-top
...
- [ ] 4.3 Stop and wait for the user's visual review before opening a PR.
