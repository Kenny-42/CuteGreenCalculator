...
### Requirement: Delete clears the current entry

**Reason**: superseded by the display becoming a real editable text field -
Delete must perform its ordinary forward-delete role instead. `CE` (the
behavior this shortcut used to trigger) remains available via its button.
**Migration**: none needed; no persisted state is affected. Users who relied
on the Delete-key shortcut for `CE` should use the `CE` button instead.
