...
- [ ] 1.1 Copy `title_bar.png`, `title_bar_pressed.png`, `logo_button.png`,
...
- [ ] 1.2 In `PixelButtonStyles.xaml`, add 72x56 (9x7 native x8) styles:
...
- [ ] 2.1 Create `Controls/TitleBarView.xaml` (496x56 design grid): background
...
- [ ] 2.2 In `TitleBarView.xaml.cs`: wire minimize/close to the hosting
...
- [ ] 3.1 In `MainWindow.xaml`: `WindowStyle="None"`, add `WindowChrome`
  ...
  (row 1, 840px).
- [ ] 3.2 In `MainWindow.xaml.cs`: update the `WM_SIZING` aspect-ratio
  constant to 496.0/896.0; wire `TitleBar.ResetRequested` to
  `Calculator.ResetDisplay()`.
- [ ] 3.3 In `CalculatorView.xaml`, remove the now-redundant internal
  `Viewbox` wrapper (root becomes the 496x840 `Grid` directly).
- [ ] 3.4 In `CalculatorView.xaml.cs`, add a public `ResetDisplay()` method
  wrapping the existing `Handle(_engine.Clear)` path.

## 4. Verification

- [ ] 4.1 `dotnet build` succeeds with no warnings/errors.
- [ ] 4.2 `dotnet test` passes with no regressions.
- [ ] 4.3 Launch the app and confirm via UI Automation + a DPI-aware
...
- [ ] 4.4 Verify drag-to-move, edge/corner resize (aspect ratio still holds),
...
- [ ] 4.5 Verify clicking the logo/daisy button resets the display the same
  way `C` does.
